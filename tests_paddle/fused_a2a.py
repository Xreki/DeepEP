...
def print_tensor_info(t, name):
    print(f"-- {name}: data_ptr={t.data_ptr():#x}, shape={t.shape}, dtype={t.dtype}, md5sum: {t._md5sum()}")
...
def fused_dispatch_forward_func(
    ...
    return recv_x, recv_token_indices, recv_token_probs, num_recv_tokens_per_expert_list, handle, event
...
def fused_combine_forward_func(
    x, group, handle, previous_event=None, async_finish=False, allocate_on_comm_stream=False
):
...
class DispatchNode:
    ...
    def forward(
        ...
        """Forward pass of fused dispatch."""
        profile_paddle.push_record_event(f"DispatchNode_forward")
        recv_x, recv_token_probs, states, event = fused_dispatch_forward_func(
            x,
            token_indices,
            token_probs,
    ...
    def backward(self, grad_output, grad_token_probs, previous_event=None, async_finish=False):
        """Backward pass of fused dispatch."""
        profile_paddle.push_record_event(f"DispatchNode_backward")
...
class CombineNode:
    ...
    def forward(self, x, group, handle, previous_event=None, async_finish=False):
        """Forward pass of fused combine."""
        profile_paddle.push_record_event(f"CombineNode_forward")
        states = dict()
        states["handle"] = handle
    ...
    def backward(self, grad_output, previous_event=None, async_finish=False):
        """Backward pass of fused combine."""
        profile_paddle.push_record_event(f"CombineNode_backward")
